...
# 出力全体で極性を固定する手法を追加した実験 exp2.jl を作成
# -------------------------------------------------------------------------
...
function run_once(n, m, m_vec, threshold)
    ...
    for i in 1:m for pola in 1:2^n if w[i][pola] == [Inf] return -1,-1,-1,-1 end end end # 閾値エラー
    ...
    # --- 共有あり ---
    w_sum_s, w_max_s = find_min_s(n, w, d_all)
...
function experiment(n, m, m_vec, k, trials)
    ...
    while count < trials
        ...
        w_n, w_s, wmn, wms = run_once(n, m, m_vec_dc, adjustable_threshold)
        if w_n == -1 # 閾値エラー
    ...
    return sum_n / trials, sum_s / trials, max_n / trials, max_s / trials
end
...
for k in ks
    avg_n, avg_s, avg_max_n, avg_max_s = experiment(n, m, m_vec, k, 1000)
    results[k] = (avg_n, avg_s, avg_max_n, avg_max_s)

    println("don't care = $k : avg(w_sum_n) = $avg_n,  avg(w_sum_s) = $avg_s")
    println("                  avg(w_max_n) = $avg_max_n, avg(w_max_s) = $avg_max_s\n")
end

println("\n=== Sum 最終結果 ===")
for k in ks
    println("DC=$k :")
    println("  non-share : sum=$(results[k][1]), max=$(results[k][3])")
    println("  share     : sum=$(results[k][2]), max=$(results[k][4])\n")
end
